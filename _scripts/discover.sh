...
SCHEMA='{"type":"object","properties":{"items":{"type":"array","items":{"type":"object","properties":{"title":{"type":"string"},"url":{"type":"string"},"source":{"type":"string"},"published":{"type":"string","description":"YYYY-MM-DD"},"summary":{"type":"string"},"role":{"type":"string","enum":["","planner","data","pm"]}},"required":["title","url","source","published","summary","role"]}},"note":{"type":"string"}},"required":["items","note"]}'
PROMPT="You are a research assistant for a Korean daily AI newsletter. Use the WebSearch tool. Budget: at most 11 searches total (batch several per turn). After the searches, stop searching and answer with the JSON — do not keep verifying.
...
- Searches (6): \"AI news $DATE\", \"OpenAI $DATE\", \"Anthropic Claude $DATE\", \"Google Gemini DeepMind $DATE\", \"AI 뉴스 $DATE\" (Korean), \"AI 발표 $DATE\" (Korean).
- Return 6-10 significant AI industry items: new models/features/policy/major launches/partnerships. Prefer official blogs and major media (1st-party reporting). role must be \"\". Date evidence may come from the URL path (e.g. /2026/07/15/), the snippet, or the result's date label; ±1 day tolerance is fine — do not drop items just because the exact hour is unknown.
